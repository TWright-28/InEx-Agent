...
Labels = ["Intrinsic", "Extrinsic", "Not a Bug", "Unknown"]
LABEL_MAP = {l.lower(): l for l in Labels}

def getAuthorInfo(obj):
    info = obj.get('author', {})
...
def formatComment(i, comment, maxLen, alwaysShowRole):
    commentUser, commentRole = getAuthorInfo(comment)
...
"""
    ...
    for comment in commentsList:
        _, commentRole = getAuthorInfo(comment)
    ...
    for i, comment in enumerate(maintainerComments, 1):
        formattedComments.append(formatComment(i, comment, 20000, alwaysShowRole=True))

    for i, comment in enumerate(regularComments[:50], len(maintainerComments) + 1):
        formattedComments.append(formatComment(i, comment, 8000, alwaysShowRole=False))
...
modelConfig = models[args.model].copy()
